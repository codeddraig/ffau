                            ...
                            selectors.forEach(selector => { // Go through each rep
                                let selectorSplit = splitIfNotInString( // Split by the
                                    splitIfNotInString( // Split by class marker (.)
                                        ...
                                        .map((z, w) => w > 0 ? "." + z : z) // Re-append the class marker (except the first)
            ...
            selectorSet.selectors.forEach((repSet, z) => { // Go through the reps
                repSet.forEach((group, b) => { // Go through the groups
                    if ([".blocklyToolboxDiv", ".blocklyTreeRoot", ".blocklyTreeRow", ".blocklyHidden",
                        ...
                        newSelectorSet.selectors[z][b] = ["div.injectionDiv.blocklyTheme" // Prepend the descendant selector
                        + cssFileName.charAt(0).toUpperCase()                             // to ensure it is only matched
                        + cssFileName.slice(1)                                            // when the given theme is used
                        + " "
                        + group.map(e => e.replace(/^\.themify */g, "")) // Add special case for `.themify`
                            .join("").trim()]; // Flatten the rest of the selector to avoid repeating the process
                });
            });
