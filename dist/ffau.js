...
class Ffau {
    ...
    addSettings(settings, autoClose) {
        ...
        let settingSections = settings.length + 1
                              ? [[null, settings]]
                              : Array.from(Object.keys(settings)).map(e => [e, settings[e]]);
    ...
    codeToXML(code) {
        ...
        */
        String.prototype.closeTag = function (tagName, closeId) {
            let openTagRE = new RegExp(
                `((?<!${nonStringId}(?:'(?:\\.|[^'])*')|(?:"(?:\\.|[^"])*"))< *${tagName}(?:(?:${nonStringId}.*?${nonStringId})|[^>])*?[^/] *>(?!< *\/ *${tagName}))`,
                'g',
            );
            ...
            return this.split(/((?:"(\\.|[^"])*")|(?:'(\\.|[^'])*'))/g)
                ...
                .map(e => openTagRE.test(e) ? `${e.trim().slice(0, -1)}/>`.replace(
                    /\/\/ *>$/g,
                    '\/>',
                ).replace(new RegExp(tagName, 'g'), tagName + closeId) : e)
                .join('');
        };
        ...
        let overrideTags = [
            'br', 'hr', 'tr', 'td', 'th', 'table', 'header', 'html', 'head', 'body', 'base', 'link', 'meta',
        ...
        let bodifiedCode = (' ' + code)
            ...
            .split(
                /(?<=<\/?[!\-a-zA-Z0-9]+(?: *[a-zA-Z]+(?:=(?:(?:"(?:\\.|[^"])*")|(?:'(?:\\.|[^'])*')|(?:[0-9]|(?:true|false))))?)* *\/?>(?:\\.|[^<])*)/g)
            .reduce((v, e) =>
                ((e.length - 1) ?
                    ...
                    case `VIDEO`:
                        ...
                        if (child.nodeName === 'AUDIO') {
                            audioSrcField.innerText = ['8bit.ogg', 'classical.mp3', 'happy.wav']
                                                          .includes(child.getAttribute('src')) ? child.getAttribute(
                                'src') : '8bit.ogg';
                        } else {
